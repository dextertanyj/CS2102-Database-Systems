...
number_of_health_declarations = 20
...
employee_id_superior = 20
employee_id_senior = 20
employee_id_manager = 40
...
for _ in range(number_of_employees):
    ...
    employee_data['department_id'].append(random.randint(0,number_of_departments - 1))
...
for _ in range(number_of_meeting_rooms):
    ...
    room = room + 1
    meeting_room_data['department_id'].append(random.randint(0,number_of_departments - 1))
...
for _ in range(number_of_health_declarations):
    id_value = random.randint(0, number_of_health_declarations - 1)
...
for _ in range(number_of_bookings):
    ...
    # Superior ID
    booking_data['creator_id'].append(random.randint(employee_id_superior, number_of_employees - 1))
    # Manager ID
    booking_data['approver_id'].append(random.randint(employee_id_manager, number_of_employees - 1))
# set_index removes the default incremental index
...
for _ in range(number_of_attendees):
    ...
    employee_id = random.randint(0, number_of_employees - 1)
...
for _ in range(number_of_updates):
    ...
    room = floor_room[1]
    date = fake.date_between(start_date=datetime.date(2021,10,1), end_date='today')
    while ((floor, room, date) in updates_pk):
        floor_room = random.choice(meeting_room_pk)
        floor = floor_room[0]
        room = floor_room[1]
        date = fake.date_between(start_date=datetime.date(2021,10,1), end_date='today')
    updates_pk.append((floor, room, date))
    updates_data['manager_id'].append(random.randint(employee_id_manager, number_of_employees - 1))
    updates_data['floor'].append(floor)
    updates_data['room'].append(room)
    updates_data['date'].append(fake.date_between(start_date=datetime.date(2021,10,1), end_date='today'))
    updates_data['capacity'].append(random.randint(0, 9))
    # set_index removes the default incremental index
df_updates_data = p
d.DataFrame(updates_data).set_index('manager_id')
